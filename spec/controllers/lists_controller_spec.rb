...
RSpec.describe ListsController, type: :controller do
  ...
  describe 'PATCH #update' do
    ...
    context 'with a change to null' do
      ...
      before do
        patch :update, format: :json, :id => list.id, :list => attributes_for(:list, :title => null_title, :description => null_description)
      end
      ...
      it 'should not change the list title in the database' do
        expect(List.find(list.id).title).not_to eq(null_title)
      end

      it 'should not change the list description in the database' do
        expect(List.find(list.id).description).not_to eq(null_description)
      end
